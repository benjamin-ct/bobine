  ...
  return (
    <div className="favorite-providers">
      <button type="button" className="chip" onClick={() => setOpen((o) => !o)}>
        {open ? "▲" : "▼"} Genres à exclure
        {excludedGenreIds.length > 0 ? ` (${excludedGenreIds.length})` : ""}
      </button>
      <p className="page-subtitle favorite-providers__hint">
        Coche les genres que tu ne veux jamais voir suggérés sur Découvrir, Nouveautés,
        Prochainement, Aléatoire et dans les recommandations d'une fiche.
      </p>
        ...
        <div className="favorite-providers__panel">
          ...
          {status === "error" && (
            <p className="page-subtitle">
              Impossible de charger la liste des genres pour le moment.
            </p>
          )}
          {status === "success" && (
